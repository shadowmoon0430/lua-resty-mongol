# Lua-Resty-Mongol Fork Project

This project is forked from [https://github.com/bigplum/lua-resty-mongol](https://github.com/bigplum/lua-resty-mongol) with modifications and additions.

### New Features:

#### `ok, err = conn:connect(host, port, opt)`

Now you can pass connection parameters `opt` to control the connection behavior. This is crucial when using connection pools, for example:

conn:connect(host, port, {pool_size = POOL_MAX, backlog = POOL_BACKLOG})

#### `db:create_view(view, source, pipeline)`
See https://www.mongodb.com/docs/v4.4/reference/method/db.createView/

#### `db:list_view(query, fields)`
#### `db:query_view(view)`
#### `db:run_command(...)`
#### `db:admin_command(...)`
Using the above two commands, you can create any functionality you want. See: https://www.mongodb.com/docs/v4.4/reference/command/

#### `col:aggregate(...)`
See: https://www.mongodb.com/docs/v4.4/reference/method/db.aggregate/

#### `col:get_indexes()`
#### `col:create_index(keys, options)`
Example:
#### `col:create_index({col_name, "idx_shard_key", {key = {_id = "hashed"}, name = "idx_shard_key"}})`
See: https://www.mongodb.com/docs/v4.4/reference/method/db.collection.createIndex/
